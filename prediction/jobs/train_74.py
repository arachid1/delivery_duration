...
from modules.models.RNN2 import RNN2
...
if __name__ == "__main__":
    ...
    for l in [1e-3, 5e-3]:
        for d in [0, 1e-4, 1e-3]:
            for b in [512, 1024, 2048]:
                parameters.lr = l
                parameters.ll2_reg = d
                parameters.batch_size = b
                print("parameters.lr: {}".format(parameters.lr))
                print("parameters.ll2_reg: {}".format(parameters.ll2_reg))
                print("parameters.batch_size: {}".format(parameters.batch_size))
                if l == 1e-3 and d == 1e-4 and b == 512:
                    print("passing")
                    continue
                train_model(RNN2)
