...
def train_model(model_to_be_trained=None):
    ...
    if parameters.stacking:
        X_hist, y_hist = stack_samples(historical_data, "market_id")
    ...
    _X_hist = np.array([item for sublist in X_hist for item in sublist])
    _y_hist = np.array([item for sublist in y_hist for item in sublist])

    window_shift = int(len(_X_hist)/parameters.n_folds)

    for w in range(0, len(_X_hist), window_shift):
        v = [y for y in range(w, w+window_shift)]
        t = list(range(0, w)) + list(range(w+window_shift, len(_X_hist)))

        X_train, X_val = _X_hist[t], _X_hist[v]
        y_train, y_val = _y_hist[t], _y_hist[v]
        # _val = q1_q2_samples[val_indexes]
        # _train = q1_q2_samples[train_indexes]
