...
def train_model(model_to_be_trained=None):
    ...
    for i in range(parameters.n_folds):
        ...
        if parameters.reduce_size:
            ...
            X_train = X_train[:int(train_length*0.2)]
            y_train = y_train[:int(train_length*0.2)]
        ...
        parameters.sequential = tf.keras.Sequential(
            [tf.keras.layers.Conv2D(
                8, kernel_size=(3, 1), padding="same", activation="relu"),
             tf.keras.layers.Conv2D(
                16, kernel_size=(3, 1), padding="same", activation="relu"),
